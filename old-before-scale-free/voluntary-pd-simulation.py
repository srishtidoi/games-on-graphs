...
class vSimulation:
    ...
    def __init__(self, population, average_degree, network_type):
        ...
        self.initial_loners = self.__choose_initial_loners()
    ...
    def __choose_initial_loners(self):
        ''' choose the initial loners (modify initial fl here)'''

        population = len(self.agents)
        rest_population = []
        for index, focal in enumerate(self.agents):
            if index not in self.initial_cooperators:
                rest_population.append(index)
                
        self.initial_loners = rnd.sample(rest_population, k = int(population/3)) #randomly choosing a third of the population to be loners
    ...
    def __initialize_strategy(self):
        ...
        for index, focal in enumerate(self.agents):
            if index in self.initial_cooperators:
                focal.strategy = 'C'
            elif index in self.initial_loners:
                focal.strategy = 'L'
            else:
                focal.strategy = 'D'
    ...
    def __play_game(self, episode, r):
        ...
        for t in range(1, tmax+1):
            ...
            if t >= 10000 and np.absolute(np.mean(fc_hist[t-100:t-1]) - fc)/fc < 0.001:
                fc_converged = np.mean(fc_hist[t-100:t])
                fl_converged = np.mean(fl_hist[t-100:t])
    ...
    def one_episode(self, episode):
        ...
        self.__choose_initial_loners()

        # Choosing values of r
        n1 = np.arange(0,0.02, 0.002)
        n2 = np.arange(0.02, 0.022, 0.0002)
        n3 = np.arange(0.022, 0.03, 0.002)
